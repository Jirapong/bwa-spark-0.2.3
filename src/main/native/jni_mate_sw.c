 ...
 jobjectArray inSeqSWArray, jobjectArray inMateSWArray, jobjectArray inRefSWArray, jintArray refSizeArray)
{
  ...
  jclass FASTQSingleNodeClass = (*env)->FindClass(env, "cs/ucla/edu/bwaspark/datatype/FASTQSingleNode");
  if(MemAlnRegTypeClass == NULL) fprintf(stderr, "[JNI/C] Class MemAlnRegType not found\n");
  else fprintf(stderr, "[JNI/C] Class FASTQSingleNode found\n");
  ...
  jfieldID seqSeqSWId = (*env)->GetFieldID(env, SeqSWTypeClass, "seq", "Lcs/ucla/edu/bwaspark/datatype/FASTQSingleNode;");
  ...
  // get field IDs: FASTQSingleNode
  jfieldID nameFASTQId = (*env)->GetFieldID(env, FASTQSingleNodeClass, "name", "Ljava/lang/String;");
  jfieldID seqFASTQId = (*env)->GetFieldID(env, FASTQSingleNodeClass, "seq", "Ljava/lang/String;");
  jfieldID qualFASTQId = (*env)->GetFieldID(env, FASTQSingleNodeClass, "qual", "Ljava/lang/String;");
  jfieldID commentFASTQId = (*env)->GetFieldID(env, FASTQSingleNodeClass, "comment", "Ljava/lang/String;");
  jfieldID seqLenFASTQId = (*env)->GetFieldID(env, FASTQSingleNodeClass, "seqLen", "I");
  jfieldID samFASTQId = (*env)->GetFieldID(env, FASTQSingleNodeClass, "sam", "Ljava/lang/String;");
  ...
  for(i = 0; i < groupSize; i++) seqLenPairs[i] = (int*) malloc(sizeof(int) * 2);
  ...
  for(i = 0; i < seqArrayLength; i++) {
    ...
    jobject seqFASTQObj = (*env)->GetObjectField(env, seqObj, seqSeqSWId);
    jstring nameFASTQStr = (*env)->GetObjectField(env, seqFASTQObj, nameFASTQId);
    const char *nameStr = (*env)->GetStringUTFChars(env, nameFASTQStr, 0);
    jstring seqFASTQStr = (*env)->GetObjectField(env, seqFASTQObj, seqFASTQId);
    const char *seqStr = (*env)->GetStringUTFChars(env, seqFASTQStr, 0);
    jstring qualFASTQStr = (*env)->GetObjectField(env, seqFASTQObj, qualFASTQId);
    const char *qualStr = (*env)->GetStringUTFChars(env, qualFASTQStr, 0);
    jstring commentFASTQStr = (*env)->GetObjectField(env, seqFASTQObj, commentFASTQId);
    const char *commentStr = (*env)->GetStringUTFChars(env, commentFASTQStr, 0);
    jint seqLenFASTQ = (*env)->GetIntField(env, seqFASTQObj, seqLenFASTQId);
    jstring samFASTQStr = (*env)->GetObjectField(env, seqFASTQObj, samFASTQId);
    const char *samStr = (*env)->GetStringUTFChars(env, samFASTQStr, 0);
    ...
    (*env)->ReleaseStringUTFChars(env, nameFASTQStr, nameStr);
    (*env)->ReleaseStringUTFChars(env, seqFASTQStr, seqStr);
    (*env)->ReleaseStringUTFChars(env, qualFASTQStr, qualStr);
    (*env)->ReleaseStringUTFChars(env, commentFASTQStr, commentStr);
    (*env)->ReleaseStringUTFChars(env, samFASTQStr, samStr);
  }
  ...
  //mem_alnreg_v** alnRegTmpVecPairs = (mem_alnreg_v**) malloc(sizeof(mem_alnreg_v*) * groupSize);
  mem_alnreg_v** alnRegVecPairs = (mem_alnreg_v**) malloc(sizeof(mem_alnreg_v*) * groupSize);
  for(i = 0; i < groupSize; i++) {
    //alnRegTmpVecPairs[i] = (mem_alnreg_v*) malloc(sizeof(mem_alnreg_v) * 2);
    alnRegVecPairs[i] = (mem_alnreg_v*) malloc(sizeof(mem_alnreg_v) * 2);
    int j = 0;
    for(j = 0; j < 2; j++) {
      //kv_init(alnRegTmpVecPairs[i][j]);
